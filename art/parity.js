...
// SVG byte-for-byte, plus rarity tier + score. Seeds 64..95 force each special type; 96..127 are founders.
...
let ok = 0, bad = 0;
for (let i = 0; i < 128; i++) {
  let seed = BigInt(keccak256(encodeAbiParameters([{ type: 'uint256' }], [BigInt(i)]))) >> 1n;
  if (i >= 96) seed |= A.FOUNDER_BIT;
  else if (i >= 64) seed = ((seed >> 16n) << 16n) | TYPE_VALUES[i % 5];
